...
OUT_DIR="${OUT_DIR:-${ROOT}/results/per_image_action_selection}"
...
run_one() {
  ...
  "$PYTHON" tools/eval_per_image_action_selection.py \
    --input_csv "$csv" \
    --label "$label" \
...
"$PYTHON" tools/summarize_per_image_action_selection.py \
  --input_dir "$OUT_DIR" \
  --out_prefix "${OUT_DIR}/per_image_action_selection_summary"

if [ -d "${ROOT}/docs/submission/full_submission/tables" ]; then
  cp "${OUT_DIR}"/per_image_action_selection_summary.* "${ROOT}/docs/submission/full_submission/tables/"
fi
